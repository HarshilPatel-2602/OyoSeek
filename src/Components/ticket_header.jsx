...
class Ticket_Header extends React.Component {
    ...
    handleSubmitReply = async () => {
        ...
        try {
            ...
            console.error("Error while submitting reply:", error.response?.data || error.message);
        }
    }
    handleMarkResolvedClick = async () => {
        const { role , status } = this.props ;
        if(role !== 'AGENT' || status !== 'Pending') return ;
        ...
        return (
            <>
                <div className="ticket-header-buttons">
                    <button className="ticket-header-buttons-reply" onClick={this.handleReplyClick} disabled={status !== 'Pending'}>
